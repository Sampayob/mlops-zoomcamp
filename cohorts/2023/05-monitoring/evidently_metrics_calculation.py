...
from evidently.metrics import (ColumnDriftMetric, ColumnQuantileMetric,
                               ConflictPredictionMetric, DatasetDriftMetric,
...
drop table if exists tripdata_metrics;
create table tripdata_metrics(
	timestamp timestamp,
...
def calculate_metrics_postgresql(curr, i):
	...
	fare_amount_quantile_value = result['metrics'][4]['result']['current']['value']

	curr.execute(
		"insert into tripdata_metrics" \
        "(timestamp, prediction_drift, num_drifted_columns, share_missing_values, num_not_stable_prediction, fare_amount_quantile_value)" \
        "values (%s, %s, %s, %s, %s, %s)",
